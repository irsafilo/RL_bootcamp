# Инструкция запуска на ноутбуке (linux + видеокарта)

1) Скачать ```/run_start/start_laptop.sh```

Выполним команду ```cat /run_start/start_laptop.sh``` , чтобы посмотреть содержимое файла.
...
```
2) Выполнить команду
  ```sudo bash /run_start/start_laptop.sh```
